...
#define BOARD_SW_3_GPIO        BOARD_SW3_GPIO
#define BOARD_SW_3_GPIO_PIN    BOARD_SW3_GPIO_PIN
#define BOARD_SW_3_PORT        BOARD_SW3_PORT
#define BOARD_SW_3_IRQ         BOARD_SW3_IRQ
#define BOARD_SW_3_IRQ_HANDLER BOARD_SW3_IRQ_HANDLER
...
#define BUTTON_1_EVENT 1 << 0
#define BUTTON_2_EVENT 1 << 1
...
static void publish_message_simp(const char *topic, const char *message, uint8_t qos);
...
uint32_t red_pin_status = 0;
uint32_t green_pin_status = 0;
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
	...
	if('1' == device_number) {
		...
		publish_message_simp("device/led_hw/1/r", message, 1);
		...
		publish_message_simp("device/led_hw/2/r", message, 1);
...
{
    static const char *topics[] = {"device/led_hw/1", "device/button/v", "device/led_hw/2"};
    int qos[]                   = {0, 0, 0};
    err_t err;
    int i;
...
static void publish_message_simp(const char *topic, const char *message, uint8_t qos) {
	...
	mqtt_publish(mqtt_client, topic, message, strlen(message), qos, 0, mqtt_message_published_cb, (void *)topic);
}
